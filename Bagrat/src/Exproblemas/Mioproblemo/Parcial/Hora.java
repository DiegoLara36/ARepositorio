...
public class Hora implements Comparable<Hora> {
     ...
     *
     * @param h parrillatv.Hora (entre 0 y 23)
     * @param m Minuto (entre 0 y 59)
     */
    //hora (hora, min)
    public Hora(int h, int m) {
        if (h < 0 || m < 0) throw new GuiaException("La hora o los minutos no pueden ser negativos");
        hh = (h + (m / 60)  ) % 24;
        mm = m % 60;
        //157%60 = 37
    }
    ...
    //8:30
    //25
    //(8+(30+25)/60)%24:: por si se pasa de la hora
    //mm = (minutos + mm)/60;9:50+40;
    //(50+40)%60=30
    /**
    ...
    /**
     * Orden natural por el que una hora es menor que otra cuando es anterior cronologicamente.
     */
    @Override
    public int compareTo(Hora hora) {
        int res = Integer.compare(hh, hora.hh);
        if (res == 0)
            res = Integer.compare(mm, hora.mm);//si la hora es la misma retorna con los minutos
        return res;
    } /// compare to por clase
    ...
    public int diferenciaMinutos(Hora hora) {
        //si la difrencia minutos es mayor que la duracion
        //8:30- 50 -----9:20
        //9:10 40 ------
        //su dif es 40 min es menor que la diracion del primero
        int minutos = hh * 60 + mm;
        int minutosHora = hora.hh * 60 + hora.mm;
        return Math.abs(minutos - minutosHora);//valor absoluto
    }
    //diferenciaminutos=hora,
